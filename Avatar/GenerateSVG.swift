...
func GenerateSVG() -> String {
    ...
    let svgStart = "<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"100%\" height=\"auto\" viewBox=\"0 0 231 231\" preserveAspectRatio=\"xMidYMid meet\">"
    let svgEnd = "</svg>"
    ...
    let svgHash: String = RandomNumber()

    // Obtiene las posiciones de cada caracter de 0 a 11 del número generado automaticamente
    let index0 = svgHash.index(svgHash.startIndex, offsetBy: 0)
    let index1 = svgHash.index(svgHash.startIndex, offsetBy: 1)
    let index2 = svgHash.index(svgHash.startIndex, offsetBy: 2)
    let index3 = svgHash.index(svgHash.startIndex, offsetBy: 3)
    let index4 = svgHash.index(svgHash.startIndex, offsetBy: 4)
    let index5 = svgHash.index(svgHash.startIndex, offsetBy: 5)
    let index6 = svgHash.index(svgHash.startIndex, offsetBy: 6)
    let index7 = svgHash.index(svgHash.startIndex, offsetBy: 7)
    let index8 = svgHash.index(svgHash.startIndex, offsetBy: 8)
    let index9 = svgHash.index(svgHash.startIndex, offsetBy: 9)
    let index10 = svgHash.index(svgHash.startIndex, offsetBy: 10)
    let index11 = svgHash.index(svgHash.startIndex, offsetBy: 11)

    var p: [String: String] = [:]

    p["env"] = String(svgHash[index0]) + String(svgHash[index1])
    p["env"] = String(Int((47.0 / 100.0 * Double(Int(p["env"] ?? "") ?? 0)).rounded()))

    p["clo"] = String(svgHash[index2]) + String(svgHash[index3])
    p["clo"] = String(Int((47.0 / 100.0 * Double(Int(p["clo"] ?? "") ?? 0)).rounded()))

    p["head"] = String(svgHash[index4]) + String(svgHash[index5])
    p["head"] = String(Int((47.0 / 100.0 * Double(Int(p["head"] ?? "") ?? 0)).rounded()))

    p["mouth"] = String(svgHash[index6]) + String(svgHash[index7])
    p["mouth"] = String(Int((47.0 / 100.0 * Double(Int(p["mouth"] ?? "") ?? 0)).rounded()))

    p["eyes"] = String(svgHash[index8]) + String(svgHash[index9])
    p["eyes"] = String(Int((47.0 / 100.0 * Double(Int(p["eyes"] ?? "") ?? 0)).rounded()))

    p["top"] = String(svgHash[index10]) + String(svgHash[index11])
    p["top"] = String(Int((47.0 / 100.0 * Double(Int(p["top"] ?? "") ?? 0)).rounded()))
    ...
    func getFinal(part: String, partV: String, theme: String) -> String {
        let colors = themes[partV]?[theme]?[part]
        let svgString = sP[partV]?[part]
        let regex = "#.*?;"
        var resultFinal = svgString ?? ""

        let result = (try? NSRegularExpression(pattern: regex))?.matches(in: svgString ?? "", range: NSRange(location: 0, length: svgString?.count ?? 0)).map { (match) -> String? in
            let range = Range(match.range, in: svgString!)
        ...
        for i in 0..<result!.count {
            resultFinal = resultFinal.replacingOccurrences(of: result![i] ?? "", with: (colors?[i] ?? "") + ";")
        }
    ...
    for part in p.keys {
        let partV = String(p[part]!.prefix(2))
        let theme = String(p[part]!.suffix(1))

        final[part] = getFinal(part: part, partV: partV, theme: theme)
    }
